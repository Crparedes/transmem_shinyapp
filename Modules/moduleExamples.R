...
examplesOutputs <- function(input, output, session) {
  ...
  output$tabSW_Na  <- renderTable(invertTrDat(seawaterLiNaK$Sodium.2), digits = 4)
  ...
  output$tabCycle1  <- renderTable(invertTrDat(reusecycles[[1]]), digits = 4)
  output$tabCycle2  <- renderTable(invertTrDat(reusecycles[[2]]), digits = 4)
  output$tabCycle3  <- renderTable(invertTrDat(reusecycles[[3]]), digits = 4)
  output$tabCycle4  <- renderTable(invertTrDat(reusecycles[[4]]), digits = 4)
  output$tabCycle5  <- renderTable(invertTrDat(reusecycles[[5]]), digits = 4)
  output$tabCycle6  <- renderTable(invertTrDat(reusecycles[[6]]), digits = 4)
  output$tabCycle7  <- renderTable(invertTrDat(reusecycles[[7]]), digits = 4)
  output$tabCycle8  <- renderTable(invertTrDat(reusecycles[[8]]), digits = 4)
  output$tabCycle9  <- renderTable(invertTrDat(reusecycles[[9]]), digits = 4)
  output$tabCycle10 <- renderTable(invertTrDat(reusecycles[[10]]), digits = 4)

  output$tabCoCycle1 <- renderTable(invertTrDat(concentrationcycles[[1]]), digits = 4)
  output$tabCoCycle2 <- renderTable(invertTrDat(concentrationcycles[[2]]), digits = 4)
  output$tabCoCycle3 <- renderTable(invertTrDat(concentrationcycles[[3]]), digits = 4)
  output$tabCoCycle4 <- renderTable(invertTrDat(concentrationcycles[[4]]), digits = 4)
  output$tabCoCycle5 <- renderTable(invertTrDat(concentrationcycles[[5]]), digits = 4)
}
